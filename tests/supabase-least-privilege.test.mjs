...
const boundary = fs.readFileSync(
  new URL('../supabase/migrations/20260819085501_service_role_boundary.sql', import.meta.url),
  'utf8'
).toLowerCase();
const atomic = fs.readFileSync(
  new URL('../supabase/migrations/20260819085607_atomic_server_rpcs_safe.sql', import.meta.url),
  'utf8'
).toLowerCase();
const hardening = fs.readFileSync(
  new URL('../supabase/migrations/20260819085712_least_privilege_service_role.sql', import.meta.url),
  'utf8'
).toLowerCase();
...
assert.match(hardening, /revoke all on tables from service_role/);
assert.match(hardening, /revoke all on table public\.rate_limit_windows_prelaunch_legacy from service_role/);
assert.match(hardening, /grant select, insert, update, delete on table[\s\S]+to service_role/);
