...
assert.equal(/\nschedule:/.test(workflow), false, 'live production/Auth/Storage/Delete verification must remain explicitly manual');
assert.match(workflow, /EXPECTED_PROJECT_REF:\s*jxmkaxwflouacuboaetg/);
assert.match(workflow, /confirm_project_ref/);
for (const action of ['verify-auth', 'verify-storage', 'verify-delete']) assert.match(workflow, new RegExp(action));
...
assert.match(workflow, /scripts\/verify-supabase-delete-live\.mjs/);
assert.match(workflow, /SUPABASE_PROJECT_REF:\s*\$\{\{ env\.EXPECTED_PROJECT_REF \}\}/);
assert.match(workflow, /SUPABASE_ACCESS_TOKEN/);
for (const action of ['verify-auth', 'verify-storage', 'verify-delete']) {
  assert.match(workflow, new RegExp(`inputs\\.action != '${action}'`));
}
...
assert.equal(workflow.includes('SUPABASE_SECRET_KEY'), false, 'modern server secret must be fetched transiently and never stored as a GitHub workflow secret');
...
console.log('OK Supabase production workflow is manual, pinned, project-locked, synthetic Auth/Storage/Delete capable and preflight-only');
