...
assert.equal(out.company_check.registry.organization_form, 'Aksjeselskap');
assert.equal(out.company_check.registry.business_activity, 'Detaljhandel');
...
assert.equal(text.includes('47.400'), false);
assert.equal(text.includes('"AS"'), false);
...
console.log('OK public paid-result projection with simple customer-safe company registry evidence');
